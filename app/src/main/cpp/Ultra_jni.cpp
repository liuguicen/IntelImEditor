...
Java_com_mandi_intelimeditor_ptu_imageProcessing_FaceSDKNative_FaceDetect(JNIEnv *env, jobject instance,
    ...
    if (imageChannel == tImageDateLen / imageWidth / imageHeight) {
        ...
        LOGE("imgSize=%d != imgW=%d * imgW=%d * imgChannel =%d", tImageDateLen, imageWidth, imageHeight,
             imageChannel);
        LOGE("img data format error");
        return NULL;
    }

    jbyte *imageDate = env->GetByteArrayElements(imageDate_, NULL);
    if (NULL == imageDate) {
        LOGE("img data is null");
        return NULL;
    }

    if (imageWidth < 50 || imageHeight < 50) {
        LOGE("img is too small");
        return NULL;
    }
