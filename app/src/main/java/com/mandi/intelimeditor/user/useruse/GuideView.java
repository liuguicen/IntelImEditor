...
//        return mContent.getSharedPreferences(TAG, Context.MODE_PRIVATE).getBoolean(generateUniqId(targetView), false);
    ...
    private void createDefaultGuideView() {
        Log.v(TAG, "createDefaultGuideView");
        // 添加到蒙层
    ...
    private void drawNoTargetBackground(Canvas canvas) {
    ...
    private void drawBackground(Canvas canvas) {
        ...
        Path path = new Path();
        path.moveTo(center[0] - targetView.getWidth() / 2f - 10, center[1] - targetView.getHeight() / 2f - 10);

        path.lineTo(center[0] - targetView.getWidth() / 2f - 10, center[1] + targetView.getHeight() / 2f + 10);
        ...
        if (myShape != null) {
            ...
            switch (myShape) {
                case CIRCULAR://圆形
                    temp.drawCircle(center[0], center[1], targetView.getWidth() / 2f, mCirclePaint);//绘制圆形
                    break;
                case ELLIPSE://椭圆
                    //RectF对象
                ...
                case RECTANGULAR://圆角矩形
                    //RectF对象
                    oval.left = center[0] - targetView.getWidth() / 2f;                              //左边
                    oval.top = center[1] - targetView.getHeight() / 2f;                                   //上边
                    oval.right = center[0] + targetView.getWidth() / 2f;                             //右边
                    oval.bottom = center[1] + targetView.getHeight() / 2f;                                //下边
    ...
    public static class Builder {
        ...
        public Builder setTips(String tips) {
            ...
            ((TextView) ll.findViewById(R.id.tv_tips)).setText(tips);
            guiderView.setCustomGuideView(ll);
            return instance;
        }
