 ...
 * @notice 类名不能改，要和后台一致
 */
public class ArtPic extends BmobObject {
...
//    public static final String SECOND_CLASS_NAME_BASE = "基本";
    ...
    public void updateHeat() {
        ...
        // 更低的概率更新，减少服务器压力
        BmobQuery<ArtPic> query = new BmobQuery<>();
        query.addWhereEqualTo("objectId", getObjectId());
        //先查询，查到了更新
        query.findObjects(
                new FindListener<ArtPic>() {
                    @Override
                    public void done(List<ArtPic> list, BmobException e) {
                        if (e != null || list == null || list.size() == 0) {
                            return;
                        }

                        ArtPic servicePR = list.get(0);
                        ...
                        heat++;
                        ArtPic.super.update(new UpdateListener() {
                            @Override
